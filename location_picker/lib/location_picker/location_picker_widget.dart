...
class _LocationPickerState extends State<LocationPicker>
  ...
  void initState() {
    ...
    if (widget.longitude != null && widget.latitude != null) {
      setState(() {
        _currentCenterCoordinate = LatLng(widget.latitude, widget.longitude);
      });
    }
  }
        ...
        children: <Widget>[
          Flexible(
            child: Stack(
              children: <Widget>[
                AmapView(
                  centerCoordinate: _currentCenterCoordinate,
                  ...
                  onMapCreated: (controller) async {
                    ...
                    // 有初始位置就不定位
                    if (_currentCenterCoordinate != null) {
                      await _search(_currentCenterCoordinate);
                      return;
                    }
  ...
  Future<void> _searchKeyword(String keyword) async {
    final poiList = await AmapSearch.instance.searchKeyword(keyword);
  ...
  Future<void> _search(LatLng location, {String keyword}) async {
    final poiList =
        await AmapSearch.instance.searchAround(location, keyword: keyword);
    _poiInfoList = poiList.map((poi) => PoiInfo(poi)).toList();
    // 默认勾选第一项
    if (_poiInfoList.isNotEmpty) _poiInfoList[0].selected = true;
    _poiStream.add(_poiInfoList);
